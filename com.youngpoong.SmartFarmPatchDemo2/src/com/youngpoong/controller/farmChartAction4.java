...
public class farmChartAction4 implements Action {
	@Override
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		ArrayList<farmDto> list = fDao.getAvgGS_Week(userId);

		if(list != null) {
			request.setAttribute("list", list);
			forward.setNextPage("farmChart6.do");
		...
		}
		{
			dDto.setId(userId);
			dDto.setColumn("차트");
			dDto.setMsg("주별 토양, 채광 차트 데이터 조회");
			dDto.setIp(ipSet);
